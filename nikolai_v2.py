...
_LOTO_CSV = Path(__file__).resolve().parent.parent / "data" / "loto7hh_4584_k23.csv"
_ap = argparse.ArgumentParser(add_help=False)
_ap.add_argument("--no-plot", action="store_true")
_NIKOLAI_V2_ARGS, _ = _ap.parse_known_args()

_raw = pd.read_csv(_LOTO_CSV, encoding="utf-8")
t = _raw[[f"Num{i}" for i in range(1, 8)]].copy()
t.columns = list(range(7))
...
# Predikcija sledećeg izvlačenja: trend poslednja − pretposlednja u istom prostoru kao ulaz u nikolai (t = sirovi brojevi)
_row_last = t.iloc[-1].to_numpy(dtype=float)
_row_prev = t.iloc[-2].to_numpy(dtype=float) if len(t) >= 2 else _row_last.copy()
_pred_raw = np.rint(_row_last + (_row_last - _row_prev)).astype(int)
for _j in range(7):
    _pred_raw[_j] = int(np.clip(_pred_raw[_j], min_val[_j], max_val[_j]))

print()
print("###############################")
print()
print("Predikcija sledećeg izvlačenja (trend na sirovim brojevima t, clip po poziciji)")
print(_pred_raw)
print(" ".join(str(int(x)) for x in _pred_raw))
print()
print()
...
plt.grid()
if not _NIKOLAI_V2_ARGS.no_plot:
    plt.show()
...
"""
python3 nikolai_v2.py
python3 nikolai_v2.py --no-plot
"""
...
"""
Predikcija sledećeg izvlačenja 
(trend na sirovim brojevima t, clip po poziciji)
[ 1  2  x  y z 38 39]
1 2 x y z 38 39
"""
...
"""
učitava istoriju izvlačenja (7 brojeva po redu), 
mapira ih u indeksirani opseg po poziciji, 
pa na tom DataFrame-u element-wise računa 
težak trigonometrijski izraz nikolai 
(sin/cos kombinacije sa koeficijentima A–E nad kolonama). 
Zatim nikolai * M, zaokruživanja, petlja i = 1..39 
gde ista formula sa skalarom i daje (i, f, fM) 
— kao pregled krive po brojevima 1–39. 
Posle toga sigmoid crtež i FFT / rFFT / shift blokovi 
nad t_ix i nikolai (spektralni prikazi). 
"""
